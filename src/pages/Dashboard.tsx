  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-r from-slate-900/60 via-slate-900/40 to-slate-950 border border-slate-800/80 p-6 shadow-xl police-border">
        <div className="absolute -right-10 -top-10 w-40 h-40 rounded-full bg-sky-500/5 blur-2xl" />
        <div className="z-10 relative flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <span className="text-[10px] bg-sky-600/10 text-sky-400 font-bold px-3 py-1 rounded-full border border-sky-500/20 uppercase tracking-widest">
              Central Operativa Geral GCM
            </span>
            <h1 className="font-outfit font-extrabold text-2xl sm:text-3xl text-slate-100 mt-2.5">
              Bem-vindo, {user?.name}
            </h1>
            <p className="text-xs text-slate-400 mt-1 max-w-xl leading-relaxed">
              Painel integrado da Guarda Civil Municipal. Efetue check-in no COPOM ou envie relatórios de PTR nos botões de acesso rápido.
            </p>
          </div>
          {hasPermission('users') && (
            <Link 
              to="/usuarios" 
              className="inline-flex items-center justify-center gap-2 px-5 py-3 rounded-xl bg-sky-600 hover:bg-sky-500 text-white text-xs font-bold transition-all shadow-lg hover:shadow-sky-500/20 uppercase tracking-wider"
            >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
        ...
        <Link to="/copom" className="glass-card rounded-xl p-5 border border-slate-800/80 hover:border-slate-700/50 transition-all flex items-center justify-between group">
          <div className="space-y-1">
            ...
            <h3 className="font-outfit font-extrabold text-2xl text-slate-200 group-hover:text-sky-400 transition-colors">
              {loading ? '...' : stats.copomCount}
            </h3>
            <p className="text-[9px] text-slate-600 font-medium uppercase tracking-wide">Acessar Frequência →</p>
          </div>
          <div className="p-3 rounded-xl bg-sky-500/5 text-sky-500 border border-sky-500/15 group-hover:bg-sky-600/10 transition-colors">
      ...
      <div className="glass-panel rounded-3xl p-6 sm:p-8 border border-sky-500/20 relative overflow-hidden bg-black shadow-[0_0_40px_rgba(234,179,8,0.05)]">
        <div className="absolute top-0 right-0 w-96 h-96 bg-sky-500/5 rounded-full blur-[100px] pointer-events-none" />
        <div className="absolute left-0 top-0 w-2 h-full bg-gradient-to-b from-sky-400 to-sky-700 shadow-[0_0_15px_rgba(234,179,8,0.5)]" />

        <div className="relative z-10 flex flex-col h-full">
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center border-b border-sky-500/20 pb-5 mb-6 gap-4">
            <div className="flex items-center gap-4">
              <div className="w-12 h-12 rounded-2xl bg-sky-500/10 flex items-center justify-center text-sky-400 border border-sky-500/30 shadow-inner">
                <Bell className="w-6 h-6" />
              </div>
              <div>
                <h3 className="font-outfit font-black text-zinc-100 text-2xl uppercase tracking-tight">Comunicações Oficiais</h3>
                <p className="text-[10px] text-sky-500 font-bold uppercase tracking-[0.2em] mt-0.5">Alto Comando do Batalhão</p>
              </div>
            </div>
            {isHighCommand && (
              <button
                onClick={() => { setShowAnnForm(!showAnnForm); setAnnError(''); setAnnSuccess(''); }}
                className="text-xs bg-sky-600 hover:bg-sky-500 text-black font-black px-4 py-2.5 rounded-xl transition-all shadow-[0_0_15px_rgba(234,179,8,0.3)] hover:shadow-[0_0_25px_rgba(234,179,8,0.5)] uppercase flex items-center gap-2"
              >
          ...
          {showAnnForm && isHighCommand && (
            <form onSubmit={handlePostAnnouncement} className="mb-8 p-6 rounded-2xl bg-zinc-900/60 border border-zinc-800 space-y-4 shadow-inner">
              <h4 className="text-xs font-black text-sky-500 uppercase tracking-[0.15em] flex items-center gap-2">
              ...
              <input
                ...
                className="w-full px-4 py-3 rounded-xl glass-input text-sm font-bold text-white placeholder-zinc-500 border-zinc-800 focus:border-sky-500"
              />
              ...
              <textarea
                ...
                rows={4}
                className="w-full px-4 py-3 rounded-xl glass-input text-sm text-zinc-300 placeholder-zinc-500 border-zinc-800 focus:border-sky-500 resize-none"
              />
          ...
          {loading ? (
            <div className="py-20 flex flex-col items-center justify-center text-zinc-600">
              <div className="w-8 h-8 rounded-full border-2 border-sky-500/20 border-t-sky-500 animate-spin mb-4" />
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              {announcements.map((ann, idx) => (
                <div key={idx} className="relative p-6 sm:p-8 rounded-2xl bg-zinc-950/80 border border-zinc-800/80 hover:border-sky-500/30 transition-all group overflow-hidden shadow-lg flex flex-col">
                  <div className="absolute top-0 left-0 w-1.5 h-full bg-gradient-to-b from-sky-600/30 to-transparent group-hover:from-sky-500 transition-colors" />
                  
                  <div className="flex justify-between items-start gap-4 mb-4">
                    <h4 className="text-lg font-outfit font-black text-zinc-100 uppercase tracking-tight flex-1 group-hover:text-sky-400 transition-colors">{ann.title}</h4>
                  ...
                  <div className="flex flex-wrap items-center gap-2 mt-auto pt-4 border-t border-zinc-900 text-[10px] uppercase font-bold tracking-widest text-zinc-500">
                    <Shield className="w-4 h-4 text-sky-500/70" />
                    <span>Autor:</span>
                    <span className="text-zinc-200">{ann.authorName}</span>
                    <span className="ml-auto px-2 py-1 rounded bg-black text-sky-500 border border-sky-900/30 text-[9px] font-black uppercase flex items-center gap-1.5 shadow-inner">
                      <RankIcon role={ann.authorRole} className="w-3.5 h-3.5 text-sky-500" />
          ...
          <div className="border-t border-zinc-900 pt-5 mt-8 flex justify-end">
            <Link 
              to="/informativos" 
              className="text-xs text-sky-500 hover:text-sky-400 font-black inline-flex items-center gap-2 group transition-colors uppercase tracking-[0.15em]"
            >
      ...
      <div className="glass-panel rounded-3xl p-6 sm:p-8 border border-zinc-800/80 bg-zinc-950/80 shadow-2xl relative overflow-hidden">
        <div className="flex items-center gap-4 border-b border-zinc-900 pb-5 mb-6">
          <div className="p-2.5 rounded-xl bg-zinc-900 text-sky-500 border border-zinc-800">
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {promotions.map((log, idx) => (
              <div key={idx} className="p-5 rounded-2xl bg-black border border-zinc-800/80 flex flex-col gap-3 hover:border-sky-500/20 transition-colors group">
                ...
                <div className="text-zinc-300 text-sm leading-snug font-medium">
                  Policial <span className="font-black text-zinc-100 group-hover:text-sky-400 transition-colors">{log.userName}</span> foi elevado na hierarquia:
                </div>
                <div className="flex items-center gap-2 mt-auto pt-2">
                  ...
                  <span className="px-2 py-1 rounded bg-sky-500/10 border border-sky-500/20 text-[9px] uppercase tracking-wider text-sky-500 font-black shadow-inner">{getRankLabel(log.newRole)}</span>
                </div>
              </div>
            ))}
          </div>
      ...
      {deleteModalOpen && (
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm z-[9999] flex items-center justify-center p-4">
          <div className="max-w-md w-full glass-panel border border-sky-500/30 bg-zinc-900 p-8 rounded-2xl text-center space-y-6 shadow-[0_0_50px_rgba(234,179,8,0.15)] animate-in fade-in zoom-in-95 duration-200 relative overflow-hidden">
            {/* sky alert top line */}
            <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-sky-600 via-sky-500 to-transparent" />
            
            <div className="w-16 h-16 rounded-full bg-sky-500/10 border border-sky-500/20 flex items-center justify-center mx-auto text-sky-500 animate-pulse">
            ...
            <div className="pt-2 flex gap-3">
              ...
              <button
                ...
                className="flex-1 py-3 px-4 rounded-xl bg-sky-600 hover:bg-sky-500 text-black font-black uppercase tracking-wider text-[10px] transition-all flex items-center justify-center gap-2 shadow-lg shadow-sky-900/40 active:scale-95"
              >
                Confirmar
              </button>
            </div>
